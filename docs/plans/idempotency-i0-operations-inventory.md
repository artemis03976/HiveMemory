...
本文是[跨子系统幂等性与重试语义计划](./cross-subsystem-idempotency-and-retry.md) **Phase I0** 的交付物。I0 的目标不是实现任何幂等机制，而是把当前所有可重试入口的现状冻结成一份可核查清单，为 I1（Work Queue 接线）、I2（Patchouli/Lifecycle 副作用）和 I3（边界验证）提供输入。
...
- **所有状态真相源都是进程内对象**，重启即丢失：
  ...
  - `MemoryGenerationTaskRegistry`（[memory_tasks.py](../../src/hivememory/patchouli/runtime/memory_tasks.py)）
  - `ExternalEventDedupRegistry`、`SealedTurnOutbox`、`MessageTurnBufferManager`、`PassiveIngressSerialGate`（[services/passive/](../../src/hivememory/system/services/passive/)）
...
Passive 是当前唯一具备显式幂等设计的入口，但承诺等级是"进程内有界 TTL"。入站 dedup 在串行门内、副作用之前；出站提交采用 outbox 顺序重试，但 outbox 不持久化。

| # | 入口（位置） | operation identity | 作用域 | 业务副作用 | 重复结果 | 并发冲突 | 模糊失败 | 幂等现状 |
|:--|:--|:--|:--|:--|:--|:--|:--|:--|
| P1 | `PassiveIngressService.ingest_event`（[passive_ingress_service.py](../../src/hivememory/system/application/passive_ingress_service.py)） | `(source, external_event_id)`（[models.py](../../src/hivememory/system/services/passive/models.py)）。**缺省时服务端生成 `pie_<uuid>`，等同无幂等** | `PassiveConversationKey`（source + external_conversation_id + user_id + agent_id + team_id） | 追加 turn buffer（内存）→ Gateway 决策 + retrieval（只读）→ seal → outbox → 提交 interaction（写记忆） | TTL 内重复 → `duplicate`，不追加 buffer/不 retrieval/不提交；TTL 外或重启 → 完整重放 | 每会话 `SerialGate` 串行 + drain 锁；无跨进程锁 | 入站可恢复失败降级（`MemoryContextAttempt(degraded=True)`，outcome 无 degraded 字段）；提交失败 outbox 保留无限重试 | 仅进程内 |
| P2 | `PassiveIngressService.flush_conversation`（[passive_ingress_service.py](../../src/hivememory/system/application/passive_ingress_service.py)） | 无显式幂等键；操作对象 = 会话当前 turn | user + agent + 外部会话 | seal 当前 buffer（manual_flush）→ drain 提交 | 重复 flush 依赖 buffer/outbox 状态近似幂等；无显式保护 | 复用 serial gate + drain 锁 | 提交失败返回 `submitted=False`，outbox 保留；无重试上限 | 缺失 |
...
| P4 | `SealedTurnSubmitter.drain`（[submitter.py](../../src/hivememory/system/services/passive/submitter.py)） | 无。`SealedTurn.turn_id` 可空、`attempts` 存在，但提交目标（Patchouli `submit_interaction`）无幂等键 | 单会话；每会话 drain 锁 | 向 Patchouli 提交完整 `InteractionPayload`（写记忆） | **缺失**：提交成功但响应丢失（模糊失败）→ 重放重复提交；outbox 重试无上限、无退避 | 每会话 drain 锁；与 serial gate 锁序一致 | 任何异常一律保留重试（不区分可重试/不可重试） | 缺失 |
| P5 | `scan_idle_conversations_once` / `shutdown_drain`（[ingressor.py](../../src/hivememory/system/services/passive/ingressor.py)） | 无；按会话 + idle 超时判定 | 全部活跃会话 | idle 超时/关停时 seal + drain | 与 P4 相同：提交无幂等键；shutdown 后 pending outbox 丢失 | 逐会话 serial gate | drain 失败保留 outbox；`outbox_pending` 非零即 `success=False` | 缺失 |

关键缺口：

- **幂等键不跨重启/不跨进程**：`ExternalEventDedupRegistry` 纯内存，崩溃/重启后 TTL 内重放被当作新事件；
- **提交目标无幂等键**：drain → Patchouli submit 全链路无 idempotency key，"提交成功但响应丢失"必然重复写入；
- **outbox 不持久化**：每会话 32 条上限，溢出丢最旧（[outbox.py](../../src/hivememory/system/services/passive/outbox.py)）是设计内数据丢失；shutdown 后 pending 项丢失；
...
| M3 | `submit_generation / submit_generation_many`（[memory_generation_tasks.py](../../src/hivememory/patchouli/control/memory_generation_tasks.py)） | `task_id = uuid4()` 每次新生成；spec 携带 `intent_id`/`pending_alias`（仅主动链路） | 进程内 `MemoryGenerationTaskRegistry`（terminal 有界 50） | 注册 task → 后台执行生成 → upsert → settlement 事件 → 写终态 | **无 intent 级去重**：同一 intent 重复提交创建多个 task，每条都会执行生成流水线 | registry 进程内 dict 无锁 | `_run_task` 捕获异常 → FAILED + best-effort `PENDING_ATOM_FAILED` | 缺失 |
| M4 | `submit_settlement / submit_active`（[memory_generation_coordinator.py](../../src/hivememory/patchouli/control/memory_generation_coordinator.py)） | 被动：`topic_id`（intent_id=None）；主动：`intent_id + pending_alias`（跨子系统稳定传递） | spec 规范化层（无状态） | 构建 `MemoryGenerationTaskSpec`；UPDATE 先 `MEMORY_GET` 校验 base 存在 | 无去重；同一 topic 重复 settle / 同一 run 重复 finalize 重复生成 spec 与任务 | 无状态天然无冲突 | `SpecBuildError` → `PENDING_ATOM_FAILED` + 跳过 | 缺失 |
...
| M6 | Task 状态机终态写入（[memory_generation_tasks.py](../../src/hivememory/patchouli/control/memory_generation_tasks.py)） | `task_id` | 进程内 registry | 终态只写一次 | `_terminal_finish_started` 标志保证第一次终态胜出；**进程崩溃后丢失** | 进程内防重 | 无（见 M5 半写入） | 仅进程内 |
| M7 | `cancel_task`（[memory_generation_tasks.py](../../src/hivememory/patchouli/control/memory_generation_tasks.py)） | `task_id` | 进程内 | request_cancel + cancel 后台 task + `PENDING_ATOM_CANCELLED` | 重复 cancel 幂等 | 无 | **关键缺口**：生成已 upsert 但被 cancel → 记忆落库却不发 SETTLED → PendingAtom 永停 MATERIALIZING | 仅进程内 |
...
| M13 | Settlement 发布（[memory_generation_tasks.py](../../src/hivememory/patchouli/control/memory_generation_tasks.py)） | `pending_alias + intent_id` | 全局事件总线 → Alice 投影 | 更新 PendingAtom 状态 + 绑定 canonical + 刷新缓存 | **at-most-once**：upsert 成功与事件发布之间崩溃 → 记忆落库但 PendingAtom 永停 MATERIALIZING；发布失败改发 FAILED | 无 | 事件丢失无重试/无 outbox | 缺失 |
