...
- [v0.7.0 计划 A](./v0.7.0-workspace-resource-system-and-agent-execution-boundaries.md)提供 Workspace 资源边界与访问能力；本计划需与其对齐 Topic、证据资源和 Actor 工作集的职责。
- [v0.7.0 计划 B](./v0.7.0-external-memory-service-and-actor-interaction.md)提供外部接入与交互契约；本计划需与其对齐上下文所有权、来源和可能的 checkpoint/上下文服务交接。
- 本次占位不改变 A/B 已有验收出口，不将完整原文保全、长 turn 管理或上下文托管自动设为 A/B 的完成前提。具体依赖、交付顺序和版本调整在详细设计时一并评估。
