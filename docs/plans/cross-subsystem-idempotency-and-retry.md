...
| HIT/CITATION/feedback | 事件类型和 best-effort 记录入口已经存在 | 同一 run、同一 citation 或 retry 是否重复计数，缺少稳定 event key 和去重策略 |
...
| HIT/CITATION/feedback | `run_id + atom_id + event_kind + event_sequence` 或显式 citation id | 同一事实事件只计一次；不同 run 的合法使用仍可累积 |
| Work item enqueue | `(lane, idempotency_key)` | 返回已有 work record，不能因为重试产生两个 handler 执行 |

上述 key 是设计起点，不是要求所有操作直接采用字符串拼接。实现时必须为 key 保存 schema/version、来源、用户作用域和结果摘要，避免未来改变输入结构后静默复用旧 key。

## 4. 目标边界

### 4.1 Producer 负责稳定身份，Consumer 负责安全副作用

Producer 生成 operation id 和幂等 key，Consumer 在自己的状态所有权内再次检查。不能只在 Gateway 或 Passive 入口去重，然后假设 Patchouli、Artifact 或 Qdrant 永远不会收到重复调用。
...
3. 让重复 HIT/CITATION/feedback 的计数规则与 event identity 一致；
...
- 同一幂等 key 的重试不会创建重复 MemoryAtom、Artifact、settlement、archive copy 或 reinforcement 副作用；
