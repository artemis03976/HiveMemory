...
| `v0.6.2` Workspace Runtime 聚合与缓存所有权迁移 | Planned | 作为 W0/W1 后续增量，建立进程级唯一 `WorkspaceRuntime`，将 Agent profile cache 与 Koakuma atom cache 纳入 Workspace-oriented 聚合并引入 Workspace-aware key；PendingAtomRuntime 暂不迁移 | 依赖 `v0.6.2` W0/W1 的 Workspace 资源边界与附件 Store；[正式 Plan](./plans/v0.6.2-workspace-runtime-cache-migration.md)；完成后更新 Workspace、System、Alice、MTP 与治理事实文档 |
