...
3.  **缓冲触发器 (Trigger)**：
    ...
    *   **注意**：当前使用偶数阈值（6条消息）以确保处理完整的 user-assistant 对话对。
    *   **TODO (后期优化)**：实现智能配对检查机制，避免在奇数阈值时将孤立的user消息传递给Patchouli。
...
*   **🏆 交付物**：一个 CLI (命令行) 聊天机器人。当你告诉它“我的 API Key 是 123”，开启新对话窗口后问它“我的 Key 是多少”，它能准确回答。
...
*   **🏆 交付物**：一个软件前端窗口。可以像使用 ChatGPT 一样使用它，并且能直观地看到右侧的“记忆侧边栏”在随着记忆的更新而实时滚动。
