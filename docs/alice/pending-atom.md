...
alias
  ...
  -> L1 atom cache（WorkspaceRuntime 持有的 KoakumaAtomCache，(WorkspaceIdentity, alias) 分区）
       返回本进程本 Workspace 已预热或曾冷查询的 MemoryAtom
...
| frame/run 收尾认领与取消 | `src/hivememory/agent_runtime/runtime.py`、`src/hivememory/alice/orchestration/run_executor.py`、`src/hivememory/alice/orchestration/sub_agent/call_coordinator.py` |
...
- PendingAtomRuntime 仍由 AliceRuntime 全局持有；L1 KoakumaAtomCache 自 v0.6.2 起由 WorkspaceRuntime 创建，alias 索引按 `(WorkspaceIdentity, alias)` 分区（UUID -> MemoryAtom 保持全局）。L0 pending 命中、L1 atom cache 命中与 L2 冷查询都已在 resolver/owner 边界重验调用方 `IdentityScope`；L0 命中作用域不匹配时按 alias 不存在处理，回归入口见 [MTP cache scope revalidation Todo](../todo/mtp-cache-scope-revalidation.md)；
