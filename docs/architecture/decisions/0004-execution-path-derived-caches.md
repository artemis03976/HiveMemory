...
v0.6.2 缓存迁移的第一阶段曾把 Alice 的两个派生缓存（L1 atom cache、Agent profile cache）移入 System 组合根的一个 `WorkspaceRuntime` 聚合，并以"共享基础设施不按 Workspace 分区原则的显式例外"论证其分区 key（该阶段性设计与记录它的分区例外 ADR 均已回退删除，实施历史见[归档 Plan](../../archive/plans/v0.6.2-workspace-runtime-cache-migration.md)）。实现后复查判定该分类的**判据**错了：
...
1. **执行子系统拥有其执行路径上的运行时状态**。L1 atom cache 与 profile cache 由 `AliceRuntime` 创建并持有（与 `PendingAtomRuntime` 同类）；第一阶段引入的 `WorkspaceRuntime` 聚合解体；
...
- "共享基础设施不按 Workspace 分区"原则恢复纯粹——派生视图缓存本就不属于该范畴，无需任何例外；
...
- 相对第一阶段的聚合实现产生一次回流（文件、导入、文档二次改写）；
- "进程级唯一"由 AliceSystem 的组装保证，而非组合根聚合对象；
- 与第一阶段相同的保留限制继续有效：profile cache 无失效事件/TTL（stale 窗口为 LRU 驻留期）、atom cache 返回原始 `MemoryAtom` 引用、两者不跨进程持久化且重启丢失。

## Alternatives

- **WorkspaceRuntime 聚合方案（第一阶段实现，已回退）**：键控结论正确，但所有权判据（key-shape）错误，制造了"共享基础设施例外"叙事与零行为聚合层；
...
- [Workspace 架构](../workspace.md)：§4.3 共享基础设施与派生缓存的键控规则；
- [System 组合根与生命周期](../../system/composition.md)、[Alice](../../alice/README.md)：所有权与生命周期的当前事实；
- [v0.6.2 Workspace Runtime 聚合与缓存所有权迁移（归档 Plan）](../../archive/plans/v0.6.2-workspace-runtime-cache-migration.md)：第一阶段聚合设计与回退的实施历史。
