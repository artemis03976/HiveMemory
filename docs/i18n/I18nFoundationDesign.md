...
HiveMemory 当前已经存在若干局部语言开关，但还没有统一的语言环境。
这会导致 Gateway、MTP、Agent prompt 组装、Retrieval 渲染、Relay 和
Memory Compiler 在 prompt 与注入文本上出现语言不一致。

当前可见问题包括：

1. Gateway 有局部配置 `gateway.analyzer.prompt_language`。
...
Memory Compiler 是后续 i18n 最关键的消费者，因为它直接决定编译后的记忆
文本如何进入 Agent 上下文。在迁移具体文案之前，系统应先建立统一的全局
语言配置与语言解析规则。
...
1. 在应用配置中增加全局语言配置。
2. 新增轻量的 `hivememory.i18n` 包，提供语言类型与语言解析能力。
3. 保留当前已有的局部语言配置，将其视为兼容性 override。
...
`normalize_language()` 对未知输入返回 `None`，不直接抛错。这样边界调用可以
保持宽容，后续如需严格校验，可在配置模型层单独增加 validator。
...
3. `component_language`：兼容历史局部配置，例如
   `gateway.analyzer.prompt_language` 或 `koakuma.mtp_prompt.language`。
...
比 Koakuma 级 MTP prompt 配置更具体，因此建议最终采用上述优先级。
...
这些字段在迁移期作为兼容性 override。待所有消费者都接入全局 i18n 后，可再
考虑将其重命名为 `language_override`，或逐步弃用。

## 5. 第一批接入点

本阶段应避免大规模迁移文案，但可以通过构造函数和 options 预留语言参数，
让后续迁移更顺畅。
...
在 `compile()` 和 `wrap()` 中，如果 `options.language` 为空，则复制 options 并
补上默认语言：
...
1. 继续保留 `gateway.analyzer.prompt_language`。
2. 后续在系统装配边界将其作为 `component_language`，与
   `config.i18n.default_language` 一起调用 `resolve_language()`。

长期可选方案：

1. 将 `prompt_language` 改为可空 override。
2. 若为空，则使用 `i18n.default_language`。

### 5.4 Relay

Relay 目前硬编码中文 prompt。它是明确的后续消费者，但可以等全局配置合入后
再接入。
...
### 5.5 Runtime 与 MTP 错误消息

Koakuma 和 KernelLoopExecutor 仍有较多英文硬编码运行时消息。本阶段不迁移，
但需要标记为后续 catalog 目标：
...
5. 修改 `AgentPromptAssembler`，接收 `default_language` 并使用
   `resolve_language()`。
6. 修改 `AliceRuntime`，传入 `config.i18n.default_language`。
7. 为 `MemoryCompileOptions` 和 `MemoryCompiler` 预留语言字段与默认语言。
...
4. `HiveMemoryConfig` 能从 YAML 加载 `i18n`，并支持环境变量覆盖：
   `HIVEMEMORY__I18N__DEFAULT_LANGUAGE=en`。
...
### 8.2 AgentProfile 与 Koakuma MTP 语言优先级不清晰
...
2. 后续消费者单独追踪：Memory Compiler、Gateway、Relay、Koakuma runtime
...
1. `i18n` 包仅包含 `types.py` 与 `resolver.py`。
...
5. 让 Retrieval renderer 和 Koakuma 将 resolved language 传入 `MemoryCompiler`。
6. 再逐步扩展到 Gateway、Relay 和 MTP runtime messages。
