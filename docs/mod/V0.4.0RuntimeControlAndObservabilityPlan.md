...
| `MemoryGenerationTask` | 一组 pending atom materialize tasks 触发的后台记忆生成 | finalize 派生 -> generation mode b/c -> settled/failed/cancelled |
...
- 保存 active chat runs 与 memory tasks。
- 管理 `asyncio.Event` cancel token。
- 提供注册、查询、取消、完成、清理接口。
- 支持按 `trace_id`、`generation_id`、`task_id` 查询。
...
`PatchouliService.finalize_agent_run()` 不再直接把 `materialize_tasks` 交给 `run_active_generation()` 并阻塞到结束，而是创建 task。
...
registry.register_memory_task(task)
await memory_task_runner.run(task)
```
...
推荐第一阶段采用方案 2：新增 RuntimeEvent SSE/WebSocket，日志和业务运行时事件分离。原因是项目后续大概率以 Electron 客户端形态落地，同时也计划提供极简 CLI 形态；因此业务事件不应绑定到日志 WebSocket，也不应只存在于 chat SSE。SSE/WebSocket/CLI JSONL 都应只是 `RuntimeEvent` 的 transport adapter。

Phase 3 的后端实现应先定义统一事件 envelope，再提供至少一种实时传输：

- Web / Electron renderer：可使用 RuntimeEvent SSE 或 WebSocket。
...
- task 状态：queued/running/cancelling/cancelled/completed/failed。
...
**实现说明**：task 包装与异步后台执行必须同步完成，不拆两步。原因：同步等待 task 完成再发 done 会导致前端 Memory Runtime Window 打开时 task 已结束，cancel 按钮无实际意义，Phase 2 的核心用户价值（"对话已完成，记忆在后台"）无法体现。`asyncio.create_task()` 本身并不复杂，task 对象与 cancel 语义才是真正的设计工作，无论同步异步都需要完成。后台 task 的异常必须在 done callback 中捕获并发出 `memory.task.failed` 事件。
...
3. Chat run、Agent frame、Memory task 发布核心事件。
4. 被动 ARCHIVE `MemoryGenerationTask` 创建后发布 `memory.task.created`，前端 Memory Runtime Window 可自动浮现。
5. 提供 RuntimeEvent transport adapter：首期可选 SSE 或 WebSocket，但不得把业务事件绑定到 chat SSE 或日志文本。
6. 前端 kernel store 分离 logs/events。
7. 修复生产 WebSocket/SSE 连接、disabled 状态和 endpoint 生成。
8. 为 Electron / CLI 预留同一事件 envelope 的 adapter 边界。
...
| memory task cancel queued | unit | task cancelled，tasks cancelled |
| memory task cancel running | integration | 当前 task 后停止，剩余 tasks cancelled |
