...
> 灏嗕竴娆＄敤鎴蜂氦浜掑強鍏舵淳鐢熺殑璁板繂鐢熸垚浠诲姟锛屽缓妯′负鍙彇娑堛€佸彲瑙傛祴銆佸彲瀹¤銆佸彲娓呯悊鐨?runtime run/task銆?
...
2. **涓诲姩璁板繂鐢熸垚鐙珛杩愯鏃?*
   - `run_active_generation()` 浠?finalize 鐨勭洿鎺ヨ繃绋嬭皟鐢紝鍗囩骇涓哄彲瑙傛祴鐨?memory generation task銆?
   - 鍓嶇澧炲姞鐙珛绐楀彛灞曠ず鍚庡彴璁板繂鐢熸垚鐘舵€併€佷换鍔″垪琛ㄣ€乼race銆侀敊璇笌浜х墿銆?
   - 鏀寔鍗曠嫭鍙栨秷 memory generation task锛屼笉褰卞搷宸茬粡瀹屾垚鐨勫璇濆洖绛斻€?
...
4. **鍙獙璇佺殑杩愯鏃跺绾?*
   - 涓?cancel銆乵emory task銆乪vent stream 澧炲姞鍗曞厓娴嬭瘯銆侀泦鎴愭祴璇曞拰鍓嶇鐘舵€佹祴璇曘€?
...
| `RuntimeEvent` | 闈㈠悜鍓嶇鍜岃皟璇曠郴缁熺殑绋冲畾鐘舵€佷簨浠?| run/task 鐢熷懡鍛ㄦ湡鍐呮寔缁彂甯?|
...
- 鍙栨秷鍓嶅彴 chat run 涓嶅簲璇潃鍏跺畠宸插瓨鍦ㄧ殑鍚庡彴 memory tasks锛岄櫎闈炶繖浜?tasks 鐢辫 run 娲剧敓涓斿皻鏈紑濮嬶紝骞朵笖绛栫暐鏄庣‘涓?cascade銆?
...
class RuntimeControlRegistry:
    ...
    def register_memory_task(self, task: MemoryGenerationTask) -> None: ...
...
| memory task 鍗曠嫭鍙栨秷 | 涓嶅奖鍝?| 鍋滄璇?task | pending atom 鏍囪 cancelled |
...
`PatchouliService.finalize_agent_run()` 涓嶅啀鐩存帴鎶?`materialize_tasks` 浜ょ粰 `run_active_generation()` 骞堕樆濉炲埌缁撴潫锛岃€屾槸鍒涘缓 task銆?

绗竴闃舵鍙互鍏堝悓姝ユ墽琛?task锛屼絾蹇呴』宸茬粡鍏峰 task 鐘舵€佷笌 cancel token锛?
...
await memory_task_runner.run(memory_task)
```

绗簩闃舵鍐嶆妸 task runner 鏀规垚鍚庡彴 task / queue銆?
...
| API | 鏂规硶 | 璇存槑 |
| :--- | :--- | :--- |
| `/api/v1/memory-tasks` | GET | 鍒楀嚭鏈€杩?娲昏穬 memory generation tasks |
| `/api/v1/memory-tasks/{task_id}` | GET | 鏌ヨ task 璇︽儏 |
| `/api/v1/memory-tasks/{task_id}/cancel` | POST | 鍙栨秷鎸囧畾 task |

濡傛灉鏆備笉鏂板 router锛屼篃鍙厛鎸傚湪鐜版湁 memory/kernel API 涓嬶紝浣嗗懡鍚嶄笂搴斾繚鐣?`task` 姒傚康銆?
...
| 閫氶亾 | 鐢ㄩ€?| 绋冲畾鎬?|
| :--- | :--- | :--- |
| RuntimeEvent | run/task 鐢熷懡鍛ㄦ湡銆佺姸鎬佹満銆佽繘搴︺€佸彇娑堛€佸け璐ャ€佷骇鐗╁紩鐢?| 绋冲畾 API |
...
- task 鐘舵€侊細queued/running/cancelling/cancelled/completed/failed銆?
...
- Events锛氭寜 run/task 灞曠ず缁撴瀯鍖栫姸鎬併€?
...
**绗簩姝?*锛歠inalize 瀹屾垚銆乵emory task 鍒涘缓骞舵敞鍐屽悗锛屽彂鍑烘渶缁?`done` 浜嬩欢锛?
...
杩欐牱 `done` 涓殑 `memory_task_ids` 鏄湡瀹炲彲鐢ㄧ殑 task id锛岃€屼笉鏄┖鍒楄〃鎴栧皻鏈瓨鍦ㄧ殑 id銆?
...
**瀹炵幇璇存槑**锛歫ob 鍖呰涓庡紓姝ュ悗鍙版墽琛屽繀椤诲悓姝ュ畬鎴愶紝涓嶆媶涓ゆ銆傚師鍥狅細鍚屾绛夊緟 task 瀹屾垚鍐嶅彂 done 浼氬鑷村墠绔?Memory Runtime Window 鎵撳紑鏃?task 宸茬粨鏉燂紝cancel 鎸夐挳鏃犲疄闄呮剰涔夛紝Phase 2 鐨勬牳蹇冪敤鎴蜂环鍊硷紙"瀵硅瘽宸插畬鎴愶紝璁板繂鍦ㄥ悗鍙?锛夋棤娉曚綋鐜般€俙asyncio.create_task()` 鏈韩骞朵笉澶嶆潅锛宩ob 瀵硅薄涓?cancel 璇箟鎵嶆槸鐪熸鐨勮璁″伐浣滐紝鏃犺鍚屾寮傛閮介渶瑕佸畬鎴愩€傚悗鍙?task 鐨勫紓甯稿繀椤诲湪 done callback 涓崟鑾峰苟鍙戝嚭 `memory.task.failed` 浜嬩欢銆?

浠诲姟锛?

1. 瀹氫箟 `MemoryGenerationTask` 妯″瀷銆?
2. `finalize_agent_run()` 鍒涘缓 task 骞舵敞鍐岋紝`asyncio.create_task()` 鍚姩鍚庡彴鎵ц锛岄殢鍗宠繑鍥烇紙涓?await锛夈€?
3. `run_active_generation()` 鎺ユ敹 task context 涓?cancel_event锛涙瘡涓?task 寮€濮嬪墠妫€鏌?cancel锛涘紓甯搁€氳繃 `memory.task.failed` 浜嬩欢涓婃姤銆?
...
3. Chat run銆丄gent frame銆丮emory task 鍙戝竷鏍稿績浜嬩欢銆?
...
| memory task cancel queued | unit | task cancelled锛宼asks cancelled |
...
1. 鍏堟妸 memory generation 鍖呮垚 task锛屼絾浠嶅彲鍚屾绛夊緟鎵ц瀹屾垚銆?
...
5. 涓诲姩璁板繂鐢熸垚鏈夌嫭绔?task id銆佺姸鎬併€佸墠绔獥鍙ｃ€?
