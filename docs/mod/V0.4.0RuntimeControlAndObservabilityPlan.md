...
Phase 3 首期最小事件集合只覆盖 `chat.run.*`、`agent.run.*`、`memory.task.*`、`memory.atom.*` 与 `event.stream.gap`。`agent.frame.*` 可作为后续扩展；除非前端开始展示 frame 树，否则不纳入 v1 必交付。
...
4. Chat run、Agent run、Memory task、Memory atom 发布核心 RuntimeEvent；其中现有 `AliceEvents.RUN_STARTED/RUN_COMPLETED` 映射为 `agent.run.started/completed`。
...
7. 保留现有功能事件（如 pending atom settled/failed/cancelled）原语义；需要展示时额外旁路转换为 RuntimeEvent，不用 RuntimeEvent 替代功能 bus。
