...
- [v0.6.2 Workspace Runtime 聚合与缓存所有权迁移](./v0.6.2-workspace-runtime-cache-migration.md)：WRT-0～WRT-5 已完成——进程级唯一 `WorkspaceRuntime` 聚合（AssetStore + 两个派生 cache + 窄化端口）落地，KoakumaAtomCache 按 `(WorkspaceIdentity, alias)`、AgentProfileCache 按 `(WorkspaceIdentity, Actor 投影, alias)` 分区，PendingAtomRuntime 保持 Alice 所有；后续经 [ADR-0005](../../architecture/decisions/0005-execution-path-derived-caches.md) 将派生缓存所有权归还 AliceRuntime、聚合解体（分区键控保留）；当前事实见 [Workspace 架构](../../architecture/workspace.md)、[System 组合根](../../system/composition.md)、[Alice](../../alice/README.md)、[MTP 契约](../../contracts/mtp.md) 与两份治理文档。
