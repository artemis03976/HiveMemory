...
> 本文保留前端视觉隐喻、交互探索和早期工作台设计过程，已停止维护。当前应用壳、主题、Chat 与实现偏差以 [Frontend 总览](../../../frontend/README.md)、[应用壳与视觉系统](../../../frontend/application-shell.md)和 [Chat 工作区](../../../frontend/chat-workspace.md)为准。本文提到的 Shadcn、可拖拽四栏和部分交互不代表当前实现。
...
**核心思想**: 以后端系统人格帕秋莉·诺蕾姬的设定为灵感，摒弃单调的冷色系暗黑模式，引入七曜魔法色彩体系（“日月交辉”与“五行水晶”）的概念，构建兼具二次元魔法感与生产力工具克制感的前端视觉体系。

***

### 1. 设计理念与目标 (Design Philosophy)

HiveMemory 系统在后端已经突破了传统“聊天机器人”的框架，演进为一套具备完整 Agent 记忆生命周期管理和专属通信协议（MTP）的类 AI 操作系统内核。因此，其前端界面绝不能仅仅是一个简单的对话框。

本阶段前端设计的核心挑战在于：**如何在一个界面中，既呈现出强大、极客的系统底座，又保持用户日常对话的流畅与专注？** 为此，我们确立了以下三大设计目标：

#### 1.1 核心目标：构建“透明、沉浸、可观测”的 AIOS 仪表盘

HiveMemory 的前端不仅是给普通用户使用的“聊天窗口（Chat Interface）”，更是给系统管理员/开发者使用的“控制台（Dashboard）”与“工作站（Workspace）”。
...
- **解法**：前端解析器（Markdown Renderer）必须充当“翻译官”。
  ...
  - **机器态视图 (Machine View)**：提供“查看原文”或右侧边栏的“Dify风格执行树”，允许高级用户随时展开卡片，偷窥底层原生的 MTP 执行流，满足极客的掌控欲。

#### 1.3 从“死黑毛玻璃”到“水晶通透感” (Crystal Immersion)

- **痛点**：深色模式背景过暗，导致毛玻璃（Glassmorphism）失去折射光源，组件显得像实心的塑料块。
...
为了在 React 项目中高效落地我们的设计理念，我们采用 **Tailwind CSS** 作为原子化样式框架，并结合 **Shadcn UI** 作为组件基础。

#### 2.1 全局环境光定义：日月交辉 (The Canvas: Sun & Moon)

宏观背景不是单一的冷色调，而是引入对角线式的冷暖碰撞，形成视觉张力。
...
#### 2.2 色彩体系：七曜魔法 (The Semantic Mapping)
...
在毛玻璃体系下，状态色必须遵循\*\*“极低透明度底色 (`bg-color/10`) + 高亮边框/文字 (`border-color/30 text-color`)”\*\*的“发光晶体”原则，绝不可使用高饱和度的实心色块。

| 元素       | 色调名称        | 状态语义        | 适用组件场景示例                                     | Tailwind / CSS 变量值 (HSL)                           |
| :------- | :---------- | :---------- | :------------------------------------------- | :------------------------------------------------- |
| **💧 水** | **湛蓝 / 青蓝** | 流动、执行中、网络请求 | MTP `[正在执行]` 卡片、流式打字机光标、Loading 动画           | `--color-magic-water: 190 90% 50%`  `(Cyan-400)`   |
| **🌲 木** | **翡翠 / 荧绿** | 生长、完成、成功、激活 | MTP `[执行成功]` 卡片、当前活跃的 Topic 绿点指示器            | `--color-magic-wood: 150 80% 45%`  `(Emerald-400)` |
| **🔥 火** | **绯红 / 玫红** | 破坏、警告、报错、删除 | MTP `[执行中断]` 卡片、Topic 卡片的删除按钮、系统 Error Toast | `--color-magic-fire: 340 80% 60%`  `(Rose-400)`    |
| **🪨 土** | **暖灰 / 陶土** | 沉淀、历史、休眠、基础 | 已 Swap-out 的归档 Topic 灰点、未激活的 Tabs 文字、历史时间戳   | `--color-magic-earth: 30 20% 50%`  `(Stone-400)`   |
| **⚡ 金**  | **白金 / 明黄** | 警示、注意、挂起、待机 | 处于 Dormant 状态的 Topic 黄点、需要用户确认的危险写入拦截        | `--color-magic-metal: 40 90% 55%`  `(Amber-400)`   |
...
#### 2.3 质感规范：水晶毛玻璃 (Crystal Glass Guidelines)

毛玻璃质感是 HiveMemory UI 的灵魂。为了避免滥用导致界面杂乱或性能下降，我们确立以下“最佳实践”：
...
2. **核心 Tailwind 类名组合 (The Glass Recipe)**
   在开发组件时，统一使用以下 Tailwind 类名组合来实现毛玻璃效果（注意：透明度已根据水晶质感调低）：
   - **主面板/侧边栏 (Heavy Glass)**:
     `bg-background/40 backdrop-blur-lg border border-white/10 shadow-xl`
   - **对话气泡/浮动卡片 (Light Glass)**:
     `bg-muted/30 backdrop-blur-sm border border-white/5`
   - **悬浮输入框 (Omni-Input)**:
     `bg-background/70 backdrop-blur-md border border-border shadow-2xl`
...
1. **卡片组件 (`Card`)**
   修改 `components/ui/card.tsx`，在默认的 `className` 中混入我们的毛玻璃配方（`bg-background/40 backdrop-blur-md`），使其默认呈现透明质感。
...
@layer utilities {
  .glass-panel {
    @apply bg-background/40 backdrop-blur-xl border border-white/10 shadow-2xl;
  }
  .glass-card {
    @apply bg-muted/30 backdrop-blur-md border border-white/5 shadow-sm hover:bg-muted/50 transition-colors;
  }
}
```
