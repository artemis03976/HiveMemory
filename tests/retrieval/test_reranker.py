...
from hivememory.retrieval.models import SearchResults, SearchResult, ProcessedQuery
...
class TestCrossEncoderReranker:
    ...
    def test_lazy_loading(self, mock_reranker_class):
        """测试模型延迟加载"""
        mock_reranker = Mock()
        mock_reranker_class.return_value = mock_reranker

        reranker = CrossEncoderReranker()
        assert reranker._model is None  # 模型未加载

        _ = reranker.model  # 访问 model 属性
        assert reranker._model is not None  # 模型已加载

    @patch("FlagEmbedding.FlagReranker")
    def test_rerank_basic(self, mock_reranker_class, sample_results, sample_query):
        """测试基本重排序功能"""
        mock_reranker = Mock()
        # 返回的分数顺序与原始结果不同，测试重排序
        mock_reranker.compute_score.return_value = [0.5, 0.9, 0.3]
        mock_reranker_class.return_value = mock_reranker
    ...
    @patch("FlagEmbedding.FlagReranker")
    def test_rerank_empty(self, mock_reranker_class, sample_query):
        """测试空结果处理"""
        mock_reranker = Mock()
        mock_reranker_class.return_value = mock_reranker
    ...
    @patch("FlagEmbedding.FlagReranker")
    def test_rerank_top_k_filtering(self, mock_reranker_class, sample_results, sample_query):
        """测试 top_k 过滤功能"""
        mock_reranker = Mock()
        mock_reranker.compute_score.return_value = [0.5, 0.9]  # 只返回2个分数
        mock_reranker_class.return_value = mock_reranker
    ...
    @patch("FlagEmbedding.FlagReranker")
    def test_rerank_no_normalization(self, mock_reranker_class, sample_results, sample_query):
        """测试不标准化分数"""
        mock_reranker = Mock()
        mock_reranker.compute_score.return_value = [0.5, 0.9, 0.3]
        mock_reranker_class.return_value = mock_reranker
        ...
        scores = [r.score for r in result.results]
        assert scores == [0.9, 0.5, 0.3]  # 直接使用原始分数

    @patch("FlagEmbedding.FlagReranker")
    def test_rerank_error_fallback(self, mock_reranker_class, sample_results, sample_query):
        """测试计算失败时的降级处理"""
        mock_reranker = Mock()
        mock_reranker.compute_score.side_effect = Exception("CUDA OOM")
        mock_reranker_class.return_value = mock_reranker
    ...
    def test_import_error_without_flagembedding(self):
        """测试 FlagEmbedding 未安装时的错误处理"""
        with patch.dict("sys.modules", {"FlagEmbedding": None}):
            with pytest.raises(ImportError, match="FlagEmbedding 未安装"):
                reranker = CrossEncoderReranker()
                _ = reranker.model

    @patch("FlagEmbedding.FlagReranker")
    def test_normalize_score(self, mock_reranker_class):
        """测试分数标准化函数"""
        mock_reranker = Mock()
        mock_reranker_class.return_value = mock_reranker
...
class TestCreateReranker:
    ...
    def test_create_with_none_config(self):
        ...
        assert isinstance(reranker, CrossEncoderReranker)
        assert reranker.config.model_name == "BAAI/bge-reranker-v2-m3"
