...
class TestIdleHibernateSwapOut:
    ...
    async def test_idle_flush_swaps_out_topic(self):
        ...
        # 验证话题在活跃池中
        active = [topic.topic_id for topic in layer._short_term_store.list_topic_data()]
        assert len(active) == 1
        ...
        active_after = [topic.topic_id for topic in layer._short_term_store.list_topic_data()]
    ...
    async def test_idle_flush_frees_slot(self):
        ...
        assert len([topic.topic_id for topic in layer._short_term_store.list_topic_data()]) == 2
    ...
    async def test_shutdown_flush_archives_and_swaps_out_all_topics(self):
        ...
        assert [topic.topic_id for topic in layer._short_term_store.list_topic_data()] == []
