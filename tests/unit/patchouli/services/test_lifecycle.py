...
class TestLifecycleFamiliar:
    ...
    async def test_refresh_memory_vitality_delegates_to_engine(self):
        ...
        lifecycle = Mock()
        lifecycle.refresh_vitality_batch = AsyncMock(return_value=[(uuid4(), 1.0)])
        familiar = self._make_familiar(lifecycle=lifecycle)
        memory = Mock(spec=MemoryAtom)

        result = await familiar.refresh_memory_vitality([memory], persist=True)

        lifecycle.refresh_vitality_batch.assert_awaited_once_with([memory], persist=True)

    @pytest.mark.asyncio
    async def test_record_hit_delegates_to_engine(self):
        lifecycle = Mock()
        lifecycle.record_hit = AsyncMock(return_value={"memory_id": "id", "delta": 0.1})
        familiar = self._make_familiar(lifecycle=lifecycle)
        memory_id = uuid4()

        result = await familiar.record_hit(memory_id, source="retrieval")

        lifecycle.record_hit.assert_awaited_once()
    ...
    async def test_record_citation_delegates_to_engine(self):
        lifecycle = Mock()
        lifecycle.record_citation = AsyncMock(return_value={"memory_id": "id", "delta": 0.05})
        familiar = self._make_familiar(lifecycle=lifecycle)
        memory_id = uuid4()

        result = await familiar.record_citation(memory_id, source="mtp")

        lifecycle.record_citation.assert_awaited_once()
    ...
    async def test_record_feedback_delegates_to_engine(self):
        lifecycle = Mock()
        lifecycle.record_feedback = AsyncMock(return_value={"memory_id": "id", "delta": 0.2})
        familiar = self._make_familiar(lifecycle=lifecycle)
        memory_id = uuid4()

        result = await familiar.record_feedback(memory_id, positive=True, source="ui")

        lifecycle.record_feedback.assert_awaited_once()
