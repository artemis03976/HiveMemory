...
class TestInMemoryShortTermStorageBasic:
    ...
    async def test_get_returns_none_for_missing(self):
        result = await self.storage.get("missing")
        assert result is None

    @pytest.mark.asyncio
    async def test_put_and_get(self):
        buf = _make_buffer("t1", "u1")
        await self.storage.put("t1", buf)

        result = await self.storage.get("t1")
    ...
    async def test_put_overwrites_existing(self):
        ...
        await self.storage.put("t1", buf1)
        await self.storage.put("t1", buf2)
    ...
    @pytest.mark.asyncio
    async def test_pop_returns_and_removes(self):
        buf = _make_buffer("t1", "u1")
        await self.storage.put("t1", buf)

        result = await self.storage.pop("t1")

        assert result is buf
        assert await self.storage.get("t1") is None

    @pytest.mark.asyncio
    async def test_pop_returns_none_for_missing(self):
        result = await self.storage.pop("missing")
        assert result is None
...
class TestInMemoryShortTermStorageList:
    ...
    @pytest.mark.asyncio
    async def test_list_by_user_returns_user_buffers(self):
        await self.storage.put("t1", _make_buffer("t1", "u1"))
        await self.storage.put("t2", _make_buffer("t2", "u1"))
        await self.storage.put("t3", _make_buffer("t3", "u2"))

        result = await self.storage.list_by_user("u1")
    ...
    @pytest.mark.asyncio
    async def test_list_by_user_returns_empty_for_no_such_user(self):
        result = await self.storage.list_by_user("missing")
        assert result == []

    @pytest.mark.asyncio
    async def test_list_all_returns_all_buffers(self):
        await self.storage.put("t1", _make_buffer("t1", "u1"))
        await self.storage.put("t2", _make_buffer("t2", "u2"))

        result = await self.storage.list_all()

        assert len(result) == 2

    @pytest.mark.asyncio
    async def test_list_all_returns_empty_when_empty(self):
        result = await self.storage.list_all()
        assert result == []
...
class TestInMemoryShortTermStorageSync:
    ...
    def test_put_sync(self):
        ...
        assert self.storage._get_sync("t1") is buf
    ...
    def test_pop_sync(self):
        ...
        assert result is buf
        assert self.storage._get_sync("t1") is None

    def test_list_by_user_sync(self):
        self.storage._put_sync("t1", _make_buffer("t1", "u1"))
        self.storage._put_sync("t2", _make_buffer("t2", "u1"))
        self.storage._put_sync("t3", _make_buffer("t3", "u2"))
    ...
    def test_list_all_sync(self):
        self.storage._put_sync("t1", _make_buffer("t1", "u1"))
        self.storage._put_sync("t2", _make_buffer("t2", "u2"))
...
class TestInMemoryShortTermStorageThreadSafety:
    ...
    def test_concurrent_put_get(self):
        ...
        def get_task(topic_id):
            try:
                for i in range(100):
                    storage._get_sync(f"{topic_id}_{i}")
            except Exception as e:
                errors.append(e)
