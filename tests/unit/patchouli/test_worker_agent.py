...
from hivememory.patchouli.worker_agent import WorkerAgentService, GenerationResult
...
class TestWorkerAgentGenerate:
    ...
    @patch("hivememory.patchouli.worker_agent.litellm.completion")
    def test_normal_completion(self, mock_completion):
        """正常完成，无 MTP 中断"""
        mock_completion.return_value = _make_response("普通回复", "stop")

        result = self.service.generate([{"role": "user", "content": "hi"}])
    ...
    @patch("hivememory.patchouli.worker_agent.litellm.completion")
    def test_mtp_interrupted(self, mock_completion):
        ...
        result = self.service.generate([{"role": "user", "content": "test"}])
    ...
    @patch("hivememory.patchouli.worker_agent.litellm.completion")
    def test_length_limited_not_mtp(self, mock_completion):
    ...
    @patch("hivememory.patchouli.worker_agent.litellm.completion")
    def test_empty_text_response(self, mock_completion):
        """LLM 返回空文本"""
    ...
    @patch("hivememory.patchouli.worker_agent.litellm.completion")
    def test_mtp_delimiter_at_start(self, mock_completion):
    ...
    @patch("hivememory.patchouli.worker_agent.litellm.completion")
    def test_multiple_delimiters_uses_last(self, mock_completion):
        """多个 ⟪ 使用 rfind 取最后一个"""
    ...
    @patch("hivememory.patchouli.worker_agent.litellm.completion")
    def test_llm_exception_propagated(self, mock_completion):
        """litellm.completion 抛异常时向上传播"""
        mock_completion.side_effect = RuntimeError("API error")

        with pytest.raises(RuntimeError, match="API error"):
            self.service.generate([{"role": "user", "content": "test"}])

    @patch("hivememory.patchouli.worker_agent.litellm.completion")
    def test_stop_sequence_injected(self, mock_completion):
