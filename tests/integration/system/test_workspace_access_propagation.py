...
def _prepared(access_context) -> PreparedAgentRun:
...
async def test_concurrent_scoped_runs_keep_independent_contexts_on_shared_service() -> None:
    ...
    async def prepare(*, access_context, **_kwargs):
        return _prepared(access_context)

    async def alice(*, agent_run_context, **_kwargs):
        return AgentRunResult(final_text=agent_run_context.access_context.workspace_identity.workspace_id)

    async def finalize(*, prepared_run, **_kwargs):
        finalized_contexts.append(prepared_run.access_context)
        return []
    ...
    main_task = asyncio.create_task(
        service.chat_scoped(
            "question",
            access_context=main_context,
            generation_id="generation-main",
        )
    )
    isolation_task = asyncio.create_task(
        service.chat_scoped(
            "question",
            access_context=isolation_context,
            generation_id="generation-isolation",
        )
    )
...
async def test_chat_rejects_prepared_run_from_different_workspace_before_alice() -> None:
    ...
    requested = make_access_context(
        user_id="u1",
        workspace_id="main_workspace",
        interaction_id="interaction-requested",
    )
    drifted = make_access_context(
        user_id="u1",
        workspace_id="isolation_workspace",
        interaction_id="interaction-drifted",
    )
    cleaned = []
    ...
    async def cleanup(*, prepared_run):
        cleaned.append(prepared_run.access_context)

    bus.register(GlobalRoutes.GATEWAY_PROCESS, gateway)
    bus.register(GlobalRoutes.PATCHOULI_PREPARE_AGENT_RUN, prepare)
    bus.register(GlobalRoutes.PATCHOULI_CLEANUP_PREPARED_AGENT_RUN, cleanup)

    with pytest.raises(WorkspaceMismatchError, match="访问上下文不一致"):
        await ChatApplicationService(bus).chat_scoped(
            "question",
            access_context=requested,
            generation_id="generation-drifted",
        )
