...
使用真实 ``InMemoryWorkspaceAssetStore`` 与 ``ShortTermMemoryStore`` 验证
design v0.6.2 §8.6 的跨 Store 竞态结果：
...
def test_remove_before_acquire_establishes_no_binding():
    ...
    assert store.list_asset_bindings(scope, topic.topic_id) == ()
...
def test_acquire_before_remove_leaves_binding_and_blocks_future_acquire():
    ...
    # 成功 Interaction 在 remove 后 commit binding：记录 remove 前已发生的真实使用。
    assert store.reserve_processing(key)
    ...
    store.release_processing(key)

    bindings = store.list_asset_bindings(scope, topic.topic_id)
...
def test_commit_before_remove_preserves_binding():
    ...
    assert len(store.list_asset_bindings(scope, topic.topic_id)) == 1
