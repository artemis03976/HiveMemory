...
从 engines/gateway/observer_buffer.py 的 ObserverSessionBuffer 演化而来，
迁入 patchouli/passive_ingest 子系统。

关键改进:
    Phase P1:
        - target_topic 与轮次绑定，修复 §3.4 路由归属风险
    Phase P2:
        - 构建结构化 turn_events (TurnEvent 列表)
        - 支持 accept_tool_call / accept_tool_result
        - flush 时产出 assistant_final_text + turn_events

状态机:
    IDLE -> (user msg) -> AWAITING_RESPONSE -> (assistant/tool msg) -> SEALED
    SEALED -> flush() -> IDLE
...
from hivememory.patchouli.passive_ingest.models import PassiveSessionKey
...
class ObserverTurnBuffer:
    ...
    Phase P2 升级要点：
    - 每次 accept_* 同时写入 _turn_events
    - flush 时 payload 包含 turn_events + assistant_final_text
    - assistant_final_text 仅由 assistant_message 事件拼接，tool 事件不参与
    """
    ...
    def accept_user(
        ...
        Returns:
            若触发了 "Next User Turn" flush，返回 (上一轮 payload, 上一轮 target_topic)
        """
        ...
        self._target_topic = (
            gaze_result.target_topic if gaze_result else None
        )
        self._turn_events.append(TurnEvent(
            kind="user_message",
        ...
        logger.debug(
            f"ObserverTurnBuffer 接收 user 消息: "
    ...
    def accept_assistant(self, content: str) -> None:
        ...
        状态转换:
        - AWAITING_RESPONSE -> SEALED
        - SEALED -> SEALED（追加）
        - IDLE -> 忽略
        """
        if self._state == ObserverBufferState.IDLE:
            logger.warning(
                f"ObserverTurnBuffer 收到 assistant 消息但无配对的 user 消息，忽略: "
                f"session={self._session_key.label}, "
        ...
        logger.debug(
            f"ObserverTurnBuffer 接收 assistant 消息: "
            f"session={self._session_key.label}, "
    ...
    def accept_tool_call(
        ...
        *,
        ...
        """
        接收 tool_call 事件 (assistant 发起的工具调用)
        ...
        if self._state == ObserverBufferState.IDLE:
            logger.warning(
                f"ObserverTurnBuffer 收到 tool_call 但无配对的 user 消息，忽略: "
                f"session={self._session_key.label}"
            )
            return
        ...
        logger.debug(
            f"ObserverTurnBuffer 接收 tool_call: "
            f"session={self._session_key.label}, "
            f"tool_name={tool_name}, action_id={action_id}"
        )
    ...
    def accept_tool_result(
        ...
        *,
        ...
        if self._state == ObserverBufferState.IDLE:
            logger.warning(
                f"ObserverTurnBuffer 收到 tool_result 但无配对的 user 消息，忽略: "
                f"session={self._session_key.label}"
            )
            return
        ...
        logger.debug(
            f"ObserverTurnBuffer 接收 tool_result: "
            f"session={self._session_key.label}, "
    ...
    def flush(self) -> Optional[FlushResult]:
        ...
        Returns:
            (InteractionPayload, target_topic) 如果有数据，否则 None
        """
        if self._state == ObserverBufferState.IDLE:
            return None
        ...
        logger.debug(
            f"ObserverTurnBuffer flush: session={self._session_key.label}"
        )
...
class ObserverTurnBufferManager:
    ...
    按 PassiveSessionKey 管理多个 ObserverTurnBuffer，线程安全。

    说明：
    - 被动入口仍可接受带 session_id 的 Identity
    - 但 session_id 只在 observer 子系统中作为外部分桶键使用
    - payload / perception 主链仍继续依赖 topic_id 进行内部隔离
    """
    ...
    def get_buffer(self, identity: Identity) -> ObserverTurnBuffer:
        ...
        with self._lock:
            if key not in self._buffers:
                self._buffers[key] = ObserverTurnBuffer(
                    identity=identity,
                    session_key=key,
                )
    ...
    def flush_idle_buffers(
        ...
        Returns:
            被 flush 的 (InteractionPayload, target_topic) 列表
        """
