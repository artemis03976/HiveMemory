...
- 认证凭据簇（``principal.py``/``credentials.py``）：调用来源身份、
  grant/有效期锚点与受控签发工厂。凭据随签发者归属本包，消费端
  （``workspace.access.WorkspaceAccessGuard`` 与各 application）只通过
  中立的结构契约与自检方法使用它们。

Workspace 侧的准入判定、行为白名单与共享行为检查由 ``hivememory.workspace``
持有；本包只编排认证，不复制授权配置，依赖方向为 system → workspace。
"""
