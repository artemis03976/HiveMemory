...
class AgentProfileConfig(BaseModel):
    ...
    allowed_mtp_verbs: List[str] = Field(
        default_factory=list,
        description="允许的 MTP 指令动词白名单，空列表=全部允许"
    )
    allowed_sys_tools: List[str] = Field(
        default_factory=list,
        description="允许的系统工具白名单，空列表=全部允许"
    )
