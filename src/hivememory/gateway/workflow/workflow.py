    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            await self._run_step(state, self._entry_step, completed_steps)
            completed_steps += 1
            ...
            for step in self._decision_prefix:
                ...
                await self._run_step(state, step, completed_steps)
                completed_steps += 1
            ...
            await self._run_step(state, analysis_step, completed_steps)
            completed_steps += 1
    ...
    async def _run_step(
        ...
        try:
            if step.timeout_ms is None:
                output = await step.invoke(selected_input)
            else:
