...
不可变 ``LogicalBlock``、估算 token、判断折叠阈值并选择待折叠 blocks。它
不持有 Store / Journal / Queue，不实现话题路由与 settle 用例（那是
...
class MemoryPerceptionEngine:
    ...
    compact 阈值判断与待折叠 blocks 选择。

    不职责：不持有 Store / Journal / Queue 等运行时状态；不实现 route /
    apply / settle 编排（Familiar 的用例）；不解释触发原因。
    """

    def __init__(self, config: SemanticFlowPerceptionConfig) -> None:
        self.config = config
