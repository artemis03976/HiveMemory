...
export function GeneralSettings({ config, updateConfig, getFieldError }: any) {
  if (!config) return null;
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <SettingSection title="系统">
        <SettingRow label="系统名称" description="HiveMemory 实例的名称。">
          <Input value={config.system.name} onChange={(v: any) => updateConfig('system.name', v)} disabled className="w-48 opacity-50 cursor-not-allowed" />
        </SettingRow>
        ...
        <SettingRow label="调试模式" description="启用详细日志和调试功能。">
          <Toggle checked={config.system.debug} onChange={(v) => updateConfig('system.debug', v)} />
        </SettingRow>
      </SettingSection>
      ...
      <SettingSection title="日志">
        ...
        <SettingRow label="控制台输出" description="将日志打印到标准输出。">
          <Toggle checked={config.logging.console_output} onChange={(v) => updateConfig('logging.console_output', v)} />
        </SettingRow>
        <SettingRow label="日志文件路径" description="保存日志文件的路径。">
          <Input value={config.logging.file_path || ''} onChange={(v: any) => updateConfig('logging.file_path', v || null)} placeholder="可选" className="w-64" />
        </SettingRow>
      </SettingSection>
    </div>
  );
